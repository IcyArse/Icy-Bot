...
async def on_message(ctx, member: discord.Member):
        ...
        try:
                ...
                embeded = discord.Embed(title="Something's wrong...", description="mentioned user doesn't exists or isn't muted", color=0x2f3136)
...
async def on_message(ctx, member: discord.Member=None):
        ...
        if member:
                greets_other = [
                        f'heyyyy {member_name}',
                        f'sup {member_name}',
                        f'well well if it isn\'t {member_name}'
                ]
                response = random.choice(greets_other)# randomly chooses a response from the greet_others list

                embeded = discord.Embed(title=response, desc="", color=0xdc8dc5)# creates a discord embed with the title and the color
        ...
        if not member:
                ...
                title = f"Heyyyy {author_name}"

                embeded = discord.Embed(title=title, desc=response, color=0xdc8dc5)
                embeded.set_image(url=response_gif)
...
async def on_message(ctx, member: discord.Member=None):
        ...
        if member:
                title = f"{author_name} wanna dance with you {member_name}"
                response = f"dance {member_name}, dance with me baby"

                embeded = discord.Embed(title=title, desc=response, color=0x7a62ad)# creates a discord embed with the title and the color
                embeded.set_image(url=response_gif)# sets the gif in the embed

                await ctx.send(embed=embeded)# sends the embed in the discord chat

        if not member:
                title = f"{author_name} is dancing"
...
async def on_message(ctx, member: discord.Member=None):
        ...
        if member:
                title = f"{author_name} is giving you a kiss {member_name}"
                response = f"*kisses*"
        ...
        if not member:
                title = f"{author_name} is kissing random people"
                response = f"*kisses*"
...
async def on_message(ctx, member: discord.Member=None):
        ...
        if member:
                avatar = member.avatar_url_as(size=256)# url for the mentioned users avatar
                title = f"here is {member_name}'s beautiful face"

                embeded = discord.Embed(title=title, color=0x2f3136)# creates a discord embed with the title and the color
        ...
        if not member:
                ...
                title = f"here's your beautiful face {author_name}"
